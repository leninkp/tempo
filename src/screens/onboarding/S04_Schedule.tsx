...
export function S04_Schedule({ onNext, onBack }: Props) {
  const [selected, setSelected] = useState([true, false, true, false, true, true, false]);
          ...
          <View style={styles.dayRow}>
            {DAYS.map((d, i) => (
              <TouchableOpacity
                key={i}
                onPress={() => toggle(i)}
                style={[styles.dayBtn, selected[i] ? styles.dayBtnOn : styles.dayBtnOff]}
                activeOpacity={0.7}
              >
                <Text style={[styles.dayLabel, { color: selected[i] ? '#000' : T.muted }]}>{d}</Text>
              </TouchableOpacity>
            ))}
          </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.sliderCard}>
            ...
            <View style={styles.track}>
              <View style={[styles.fill, { width: `${((duration - 15) / 75) * 100}%` }]} />
              <View style={[styles.thumb, { left: `${((duration - 15) / 75) * 100}%` }]} />
            </View>
...
const styles = StyleSheet.create({
  ...
  durationVal: { fontSize: 34, fontWeight: '600', color: T.text, letterSpacing: -1 },
